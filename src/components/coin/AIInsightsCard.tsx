...
export const AIInsightsCard: React.FC<AIInsightsCardProps> = ({
  ...
  return (
    <div className="bg-gradient-to-br from-blue-600/20 to-indigo-900/20 border border-blue-500/20 rounded-2xl p-6 my-6 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 border-b border-slate-800/60 pb-4">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-blue-600/30 border border-blue-500/30 text-blue-400 flex items-center justify-center">
        ...
        <button
          onClick={loadAI}
          disabled={loading}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900/50 hover:bg-slate-800/60 border border-slate-800 text-xs font-medium text-slate-300 hover:text-white transition-colors cursor-pointer self-start sm:self-auto disabled:opacity-50"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs font-mono text-slate-300 bg-slate-900/50 px-3 py-1 rounded-lg border border-slate-800">
              AI Confidence: <strong className="text-blue-400">{analysis.confidenceScore}%</strong>
            </span>
          </div>

          {/* Executive Summary */}
          <p className="text-sm text-slate-300 leading-relaxed bg-slate-900/40 p-4 rounded-xl border border-slate-800/60 font-sans">
            {analysis.summary}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {analysis.keyTakeaways && analysis.keyTakeaways.length > 0 && (
              <div className="bg-slate-900/40 border border-slate-800/60 rounded-xl p-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-green-500 font-mono mb-2 flex items-center gap-1.5">
            ...
            {analysis.riskFactors && analysis.riskFactors.length > 0 && (
              <div className="bg-slate-900/40 border border-slate-800/60 rounded-xl p-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-red-500 font-mono mb-2 flex items-center gap-1.5">
          ...
          {analysis.technicalOutlook && (
            <div className="bg-slate-900/40 border border-slate-800/60 rounded-xl p-3 text-xs text-slate-400 flex items-center gap-2 font-mono">
              <TrendingUp size={14} className="text-blue-400 shrink-0" />
              <span>
                <strong className="text-slate-200">Technical Momentum:</strong> {analysis.technicalOutlook}
              </span>
            </div>
          )}
        </div>
