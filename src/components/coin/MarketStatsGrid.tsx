...
export const MarketStatsGrid: React.FC<MarketStatsGridProps> = ({ marketData, symbol }) => {
  ...
  return (
    <div className="bg-[#14171F] border border-slate-800/60 rounded-2xl p-6 my-6 shadow-2xl">
      <h3 className="text-lg font-bold text-white tracking-tight mb-4 flex items-center gap-2 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-4">
          ...
          <div className="text-lg font-bold font-mono text-white">{formatCurrency(marketCap, 0, true)}</div>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-4">
          ...
          <div className="text-lg font-bold font-mono text-white">{formatCurrency(fdv, 0, true)}</div>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-4">
          ...
          <div className="text-lg font-bold font-mono text-white">{formatCurrency(volume24h, 0, true)}</div>
          <div className="text-[11px] text-slate-400 mt-0.5 font-mono">Vol / Cap: {volumeToCapRatio}</div>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-4">
          ...
          <div className="text-sm font-bold font-mono text-white flex items-center justify-between">
            <span className="text-green-500">{formatCurrency(high24h)}</span>
            <span className="text-slate-500">/</span>
            <span className="text-red-500">{formatCurrency(low24h)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t border-slate-800/60">
        ...
        <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-5 space-y-3">
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300 font-mono flex items-center gap-1.5">
            <Database size={14} className="text-blue-400" /> Token Supply Dynamics
          </h4>
          ...
          <div className="flex justify-between text-xs py-1 border-b border-slate-800/40">
            ...
            <span className="font-mono text-white font-bold">
              {formatNumber(circulating)} <span className="text-slate-400 uppercase">{symbol}</span>
            </span>
          </div>
          ...
          <div className="flex justify-between text-xs py-1 border-b border-slate-800/40">
            ...
            <span className="font-mono text-white font-bold">
              {formatNumber(total)} <span className="text-slate-400 uppercase">{symbol}</span>
            </span>
          </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-5 space-y-3">
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300 font-mono flex items-center gap-1.5">
            <Coins size={14} className="text-amber-400" /> All-Time Records
          </h4>

          <div className="flex items-center justify-between text-xs py-1 border-b border-slate-800/40">
            <div>
              <span className="text-slate-400 block text-[11px]">All-Time High (ATH)</span>
              <span className="font-mono font-bold text-white text-sm">{formatCurrency(ath)}</span>
              <span className="text-[10px] text-slate-500 block">{formatDate(athDate)}</span>
            </div>
            <span className="font-mono text-red-500 font-bold bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20">
          ...
          <div className="flex items-center justify-between text-xs py-1">
            <div>
              <span className="text-slate-400 block text-[11px]">All-Time Low (ATL)</span>
              <span className="font-mono font-bold text-white text-sm">{formatCurrency(atl)}</span>
              <span className="text-[10px] text-slate-500 block">{formatDate(atlDate)}</span>
            </div>
            <span className="font-mono text-green-500 font-bold bg-green-500/10 px-2 py-0.5 rounded border border-green-500/20">
