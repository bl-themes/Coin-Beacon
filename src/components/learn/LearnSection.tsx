  ...
  return (
    <div className="my-8">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-pulse">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-48 bg-slate-900/40 rounded-2xl p-6 border border-slate-800/60"></div>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {articles.map((art) => (
            <div
              ...
              className="bg-[#14171F] border border-slate-800/60 rounded-2xl p-6 hover:border-blue-500/40 hover:bg-slate-900/40 transition-all cursor-pointer flex flex-col justify-between group shadow-2xl"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-slate-900/50 border border-slate-800/60 flex items-center justify-center shrink-0">
                    {getIcon(art.icon)}
                  </div>
                  <span
      ...
      {selectedArticle && (
        ...
        >
          <div
            className="w-full max-w-2xl bg-[#14171F] border border-slate-800/60 rounded-2xl shadow-2xl p-6 max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-150"
          ...
          >
            <div className="flex items-center justify-between gap-4 border-b border-slate-800/60 pb-4 mb-4">
              ...
              <button
                onClick={() => setSelectedArticle(null)}
                className="p-1 rounded-lg bg-slate-900/50 text-slate-400 hover:text-white cursor-pointer"
              >
                <X size={18} />
              </button>
            </div>

            <h2 className="text-xl sm:text-2xl font-bold text-white mb-4 leading-tight font-sans">
